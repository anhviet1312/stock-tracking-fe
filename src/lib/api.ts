...
export async function fetchStockHistory(symbol: string, resolution: string, from: number, to: number): Promise<StockHistoryData | null> {
  try {
    const response = await fetch(`${API_BASE_URL}/stocks/history/${symbol}?resolution=${resolution}&from=${from}&to=${to}`);
    if (!response.ok) {
...
export async function getAlerts(): Promise<UserStockAlert[]> {
  const token = localStorage.getItem('auth_token');
  if (!token) throw new Error("Unauthorized");
  
  const response = await fetch(`${API_BASE_URL}/protected/alerts`, {
    headers: { 'Authorization': `Bearer ${token}` }
  });
...
export async function setAlert(symbol: string, highThreshold?: number, lowThreshold?: number): Promise<void> {
  const token = localStorage.getItem('auth_token');
  if (!token) throw new Error("Unauthorized");
  ...
  const json = await response.json();
  if (!response.ok || (json?.code && json.code !== 'SUCCESS')) {
    throw new ApiError(json?.message || 'Failed to set alert', json?.code);
  }
}
...
export async function removeAlert(id: string): Promise<void> {
  ...
  const response = await fetch(`${API_BASE_URL}/protected/alerts/${id}`, {
    method: 'DELETE',
    headers: { 'Authorization': `Bearer ${token}` }
  });
  ...
  const json = await response.json();
  if (!response.ok || (json?.code && json.code !== 'SUCCESS')) {
    throw new ApiError(json?.message || 'Failed to remove alert', json?.code);
  }
}
...
export async function updateTelegramChatId(telegramChatId: string): Promise<void> {
  ...
  const response = await fetch(`${API_BASE_URL}/protected/users/telegram`, {
    method: 'PUT',
    headers: { 
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json' 
  ...
  const json = await response.json();
  if (!response.ok || (json?.code && json.code !== 'SUCCESS')) {
    throw new ApiError(json?.message || 'Failed to update telegram chat id', json?.code);
  }
}
